...
*   **Q: 如何修改最大矩阵尺寸？**
    *   A: 目前 `matrix_mem.v` 中的 `MEM_DEPTH` 为 96，地址计算逻辑支持最大约 8x8 或 10x10 的矩阵（取决于行地址位宽）。如需更大矩阵，需修改存储器深度和地址位宽定义。
*   **Q: 卷积模块处理的图像在哪里？**
    *   A: 图像数据硬编码在 `bonus_conv.v` 内部的 `input_image_rom` 模块中，用于演示目的。实际应用需修改为从外部或存储器读取图像。
