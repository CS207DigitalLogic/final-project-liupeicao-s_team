...
47.    *   串口发送: `1 2 3 4 5 6` (6个元素，空格或回车分隔。**注意**: 模拟器中需一次性发送，实物可分次)。
48.    *   **现象**: 输入完成后，系统自动回到 IDLE。
...
> **话术**: “现在演示错误处理。我尝试将 A (2x3) 与 B (3x2) 相加，这违反了维度规则。同时演示在倒计时结束前快速修正错误的机制。”

1.  保持 `SW[7:5]=011` (Calc Mode)。
2.  拨动 `SW[4:2]=000` (加法 Opcode)。
3.  按下 `Btn_C` (进入 Select Matrix 状态)。
4.  **故意错误**: 此时系统尝试 A+B (2x3 + 3x2)，默认 Slot A(00) 和 Slot B(01)，维度不匹配。
5.  按下 `Btn_C` (Confirm)。
6.  **现象**:
    *   数码管显示 `Err` 和倒计时 (从 5 开始，因为第一步设置了 5秒)。
    *   LED 闪烁。
7.  **操作 (加分点)**: 不用等倒计时结束！
    *   **快速修正**: 拨动 `SW[3:2]=00` (将第二个操作数改为 Slot A)。
    *   此时运算变为 A + A (2x3 + 2x3)，维度匹配，是合法的！
    *   解释说“啊，我选错矩阵了，应该是 A 加 A”，然后再次按下 `Btn_C`。
    *   **现象**: 系统立即跳出错误状态，进入计算并显示结果。
    *   *(注: 我们的设计允许在报错倒计时期间直接修改操作数 Slot 并重试，无需等待超时)*。
...
## 4. 附录：操作数与开关映射 (Operand Mapping)

在**计算模式 (Calc Mode)** 下，当不进行标量运算时，开关用于选择参与运算的矩阵（操作数）：

*   **SW[1:0]**: 选择 **第一个操作数 (Operand 1)**
    *   `00`: Slot A (矩阵 A)
    *   `01`: Slot B (矩阵 B)
    *   `10`: Slot C (结果矩阵)
*   **SW[3:2]**: 选择 **第二个操作数 (Operand 2)**
    *   `00`: Slot A
    *   `01`: Slot B
    *   `10`: Slot C

例如：
*   计算 `A + B`: 设置 `SW[1:0]=00` (A), `SW[3:2]=01` (B)。
*   计算 `A + A`: 设置 `SW[1:0]=00` (A), `SW[3:2]=00` (A)。
